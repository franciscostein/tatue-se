...
exports.authenticate = async (email, password) => {
    const invalidCredentialsError = apiResponsePayloadName(
        'errors',
...
exports.sendResetPasswordEmail = async emailAddress => {
    ...
    if (sent) {
        return apiResponse({
            msg: 'E-mail sent! Please check your inbox for instructions.',
        });
    } else {
        throw new Error(
            'There was an error. E-mail not sent, please try again.'
        );
    }
};
...
exports.resetPassword = async (userId, token, newPassword) => {
    ...
    try {
        ...
        return apiResponse(
            { msg: 'Token expired or invalid, please request another link.' },
            401
        );
    }
